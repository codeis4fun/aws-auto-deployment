...
logger.info(f'Listando objetos do bucket {BUCKET_NAME} em {LANDING_BUCKET}')
objects = s3.list_objects(LANDING_BUCKET)
sqs = SQS(REGION, ACCOUNT_ID, SQS_QUEUES[0])
logger.info(f'Enviando os objetos encontrados para a fila {SQS_QUEUES[0]} do SQS')
